...
//#include "cc_common_common.h"
//#include "utility.h" 

#define ENABLED 0


int64_t ts_start_of_xds = -1; // Time at which we switched to XDS mode, =-1 hasn't happened yet
...
struct ccaptionXDS_dec_context *ccaptionXDS_dec_init(
    struct cc_common_timing_ctx *timing, 
    int xds_write_to_file){

	int i;
	struct ccaptionXDS_dec_context *ctx = NULL;

	ctx = malloc(sizeof(struct ccaptionXDS_dec_context));
	if(!ctx)
	    return NULL;
...
#if ENABLED
int write_xds_string(
    struct cc_subtitle *sub,
    struct ccaptionXDS_dec_context *ctx,
    char *p, size_t len){
...
void xdsprint (struct cc_subtitle *sub, struct ccaptionXDS_dec_context *ctx, const char *fmt,...)
{
...
void xds_debug_test(struct ccaptionXDS_dec_context *ctx, struct cc_subtitle *sub)
{
...
void xds_cea608_test(struct ccaptionXDS_dec_context *ctx, struct cc_subtitle *sub)
{
...
int how_many_used(struct ccaptionXDS_dec_context *ctx)
{
	int c=0;
...
static void clear_xds_buffer (struct ccaptionXDS_dec_context *ctx, int num){
...
void process_xds_bytes (struct ccaptionXDS_dec_context *ctx,
	...
	//	ccx_common_logging.debug_ftn(CCX_DMT_DECODER_XDS, "XDS: %02X.%02X (%c, %c)\n",hi,lo,hi,lo);
		if ((hi>0 && hi<=0x1f) || (lo>0 && lo<=0x1f)){
            ...
            ctx->fsd->cc608_dp.xds_invalid_characters = 1;
			return;
		}
	}
 ...
 */
#if ENABLED
static void xds_do_copy_generation_management_system (struct cc_subtitle *sub, 
#else
static void xds_do_copy_generation_management_system ( 
#endif
    struct ccaptionXDS_dec_context *ctx, unsigned c1, unsigned c2){
	...
	if (last_c1!=c1 || last_c2!=c2){
		...
		const char *copytext[4]={
            "Copy permited (no restrictions)"      , "No more copies (one generation copy has been made)" ,
			"One generation of copies can be made" , "No copying is permited"
        };
		
        const char *apstext[4]={
            "No APS"                        , "PSP On; Split Burst Off"         ,
            "PSP On; 2 line Split Burst On" , "PSP On; 4 line Split Burst On"
        };
...
#if ENABLED
static void xds_do_content_advisory (struct cc_subtitle *sub,
#else
static void xds_do_content_advisory ( 
#endif       
    struct ccaptionXDS_dec_context *ctx,
	...
	if (last_c1!=c1 || last_c2!=c2){
		...
		if (!a1 && a0){ // US TV parental guidelines
		    const char *agetext[8]={
                "None"                               , "TV-Y (All Children)"                 , "TV-Y7 (Older Children)" ,
		...
		if (a0 && a1 && !Da2 && !La3){ // Canadian English Language Rating
			const char *ratingtext[8]={
                "Exempt"                                         , "Children"          , "Children eight years and older" ,
...
#if ENABLED
static int xds_do_current_and_future (struct cc_subtitle *sub, 
    struct ccaptionXDS_dec_context *ctx){
#else
static int xds_do_current_and_future (
    struct ccaptionXDS_dec_context *ctx){
#endif 	
    int was_proc = 0;
...
#else
            xds_do_content_advisory (ctx, ctx->cur_xds_payload[2],ctx->cur_xds_payload[3]);
#endif
			break;
...
#else
			xds_do_copy_generation_management_system (ctx, ctx->cur_xds_payload[2],ctx->cur_xds_payload[3]);
#endif
			break;
		case XDS_TYPE_ASPECT_RATIO_INFO:
        {
            unsigned int ar_start, ar_end;
            int changed;
            changed = 0;
            was_proc    = 1;
            if (ctx->cur_xds_payload_length < 5) // We need 2 data bytes
...
#if ENABLED
static int xds_do_channel (struct cc_subtitle *sub, 
#else
static int xds_do_channel (
#endif
    struct ccaptionXDS_dec_context *ctx){
	int was_proc=0;
	if(!ctx)
...
#if ENABLED
static int xds_do_private_data (struct cc_subtitle *sub, 
#else
static int xds_do_private_data ( 
#endif
    struct ccaptionXDS_dec_context *ctx) {
	char *str;
	int i;
...
static int xds_do_misc (struct ccaptionXDS_dec_context *ctx){
	int was_proc=0;
	if(!ctx)
		return CC_EINVAL;
...
//		ccx_common_logging.debug_ftn(CCX_DMT_DECODER_XDS, "Expected checksum: %02X  Calculated: %02X\n", expected_checksum, cs);
        ...
        ctx->fsd->cc608_dp.xds_checksum_error = 1;
...
#else
            was_proc = xds_do_current_and_future(ctx);
#endif
			break;
...
#else
			was_proc = xds_do_private_data(ctx);
#endif
			break;
	...
	if (!was_proc){
        ctx->fsd->cc608_dp.xds_invalid_pkt_structure = 1;
